    ...
    <Listbox value={picked} onChange={setPicked}>
      <div className="relative">
        ...
        <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
          <Listbox.Options className="absolute right-0 z-50 mt-2 w-72 overflow-auto rounded border border-zinc-300 bg-zinc-100 py-2 shadow xl:w-72">
              ...
              <Listbox.Option
                ...
                className={({ active }) =>
                  `relative cursor-pointer select-none py-1.5 pl-10 pr-5 text-sm font-normal tracking-tight ${
                    active ? 'bg-primary/10 text-primary dark:bg-tertiary/10 dark:text-tertiary' : 'text-zinc-800'
                  }`
                }
                value={option}
              >
